...
## Роль канала (решено 2026-08-10)

IG — **employer branding и «очеловечивание» бренда, не лидоген**, и
**уникального продакшена не получает**. Контент приходит репёрпосом из ядровой
единицы недели проекта `smm` (соседняя папка рабочего каталога): карусели из
дайджеста, Reels из тех же скринкастов, что идут в Shorts и TikTok.
Собственного контент-плана у канала больше нет — решение и модель в
`smm/docs/plan.md` §2.6 и `smm/docs/pipeline.md`.

Что это меняет для кода: этот проект остаётся **публикатором и инбоксом**.
Рендерер каруселей (`lib/render.ts`, `lib/slides.ts`), `publish-carousel.ts` и
живой токен в Upstash — здесь, и переносить их в `smm` не надо. `smm` передаёт
только текст слайдов и подпись через `smm/hub/<week>/outbox/*.json`.

Брифы в `docs/` (`instagram-content-plan.md`, `instagram-execution.md`,
`reels-ideas.md`, `avatar-reels-automation.md`, `instagram-brand-plan-sepia.md`)
написаны **до** этого решения и описывают IG-first продакшен с каденсом 4 reels
в неделю. У каждого сверху стоит шапка о смене роли. Как план их не выполнять;
фактура про бренд, тон и профиль в них в силе. Снятые задачи — в корневом
`TODO.md` → Backlog.
